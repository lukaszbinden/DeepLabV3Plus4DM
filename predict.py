...
def main():
    ...
    with torch.no_grad():
        ...
        for img_path in tqdm(image_files):
            ...
            #     file = os.path.join(opts.save_val_results_to, img_name+'.png')
            #     colorized_preds.save(file)
            #     print(f"written output to {file}...")
            preds_grayscale = Cityscapes.map_train_id_to_id(pred).astype('uint8')
            preds_grayscale = Image.fromarray(preds_grayscale)
            if opts.save_val_results_to:
                file = os.path.join(opts.save_val_results_to, img_name+'.png')
                preds_grayscale.save(file)
                print(f"written output to {file}...")
